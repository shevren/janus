...
export async function runGithub(userId: string, name: string, args: Record<string, unknown>): Promise<string> {
  ...
  if (name === "github_create_repo") {
    ...
    const isPrivate = args.private === true || String(args.private) === "true";
    const out = await gh(userId, "/user/repos", {
      method: "POST",
      body: JSON.stringify({ name: repoName, description, private: isPrivate, auto_init: true }),
    });
    const url = (out as { html_url?: string }).html_url ?? "";
    return `Repository created: ${url}${isPrivate ? " (private)" : ""}`;
  }
  if (name === "github_write_files") {
    ...
    for (const f of files.slice(0, 20)) {
      ...
      const body = { message: `add ${f.path}`, content };
      await gh(userId, `/repos/${owner}/${repoName}/contents/${f.path}`, {
        method: "PUT",
        body: JSON.stringify(body),
      });
      written++;
    }
    return `Wrote ${written} files to ${owner}/${repoName}`;
  }
  if (name === "github_deploy") {
    const repoName = String(args.repo ?? "");
    const platform = String(args.platform ?? "pages");
    if (platform === "pages") {
      const out = await gh(userId, `/repos/${owner}/${repoName}/pages`, {
        method: "POST",
        body: JSON.stringify({ source: { branch: "main", path: "/" } }),
      });
      return `GitHub Pages deployed: ${(out as { html_url?: string }).html_url ?? ""}`;
    }
    return `Deploy to ${platform} not implemented. Use "pages" for free hosting.`;
  }
...
export function githubTools() {
  return [
    ...
    },
    {
      ...
      description: "Create a new GitHub repository (public or private). Returns the repo URL.",
      parameters: {
        type: "object",
        properties: {
          name: { type: "string" },
    ...
    },
    {
      ...
      description: "Write multiple files to a repository. Each file has path and content.",
      parameters: {
        type: "object",
        properties: {
          owner: { type: "string" },
    ...
    {
      name: "github_deploy",
      description: "Deploy repository to free hosting (pages). Returns deployed URL.",
      parameters: {
        type: "object",
        properties: { owner: { type: "string" }, repo: { type: "string" }, platform: { type: "string" } },
        required: ["owner", "repo"],
      },
    },
    {
